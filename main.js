...
function updatePayBtnUI() {
    ...
    if(currentPlansView === 'monthly') {
        ...
        <button class="btn btn-stars" id="pay-stars-btn">
            <i class="fas fa-star"></i> <span data-i18n="pay_stars_btn">Оплатить Stars</span>
            <span class="stars-badge" id="stars-badge-amount">0 ⭐</span>
        </button>

        <button class="btn btn-crypto" id="open-crypto-btn">
            <i class="fas fa-coins"></i> <span data-i18n="pay_crypto_btn">Pay with Crypto</span>
        </button>

        <button class="btn btn-crypto" id="open-card-to-card-btn">
            <i class="fas fa-credit-card"></i>Card-to-Card
        </button>
        `;
    }
...
window.showPaymentModal = (toman, type, months, price, stars) => {
    ...
    ? `
        <div class="modal-info-row"><span>مبلغ کارت به کارت</span><strong>${toman.toLocaleString()} تومان</strong>
        </div>
      `
    : '';
...
function updatePlansUI() {
    ...
    if(currentPlansView === 'monthly') {
        ...
        subtitle.innerHTML =
            'Choose a National Internet package with Card-to-Card support.';

        methods.innerHTML = `
        <div class="pm-badge stars"><i class="fas fa-star"></i> Telegram Stars</div>
        <div class="pm-badge yoo"><i class="fas fa-coins"></i> Crypto</div>
        <div class="pm-badge yoo"><i class="fas fa-credit-card"></i> Card-to-Card</div>
        `;

        warning.innerHTML =
            'Card-to-card payments are verified manually after payment.';
    }

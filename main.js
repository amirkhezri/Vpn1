...
function updatePayBtnUI() {
    ...
    if(currentPlansView === 'monthly') {
        ...
        <button class="btn btn-stars" id="pay-stars-btn">
            <i class="fas fa-star"></i> <span data-i18n="pay_stars_btn">پرداخت با استارز </span>
        ...
        <button class="btn btn-toman" id="open-card-to-card-btn">
            ...
            <span class="toman-badge" id="toman-badge-amount">0 $</span>
        </button>
        `;
    }
...
window.showPaymentModal = (toman, type, months, price, stars) => {
    ...
    ? `
        <div class="modal-info-row"><span>مبلغ کارت به کارت</span><strong>تومان ${toman.toLocaleString()} </strong>
        </div>
      `
    : '';
    ...
    if(isVolume){
        ...
        document.getElementById('toman-badge-amount').textContent = `تومان ${toman.toLocaleString()} `;
    }else{
        document.getElementById('stars-badge-amount').textContent = `${stars} ⭐`;
        document.getElementById('crypto-badge-amount').textContent = `${price.toFixed(2)} $`;
    }
...
function updatePlansUI() {
    ...
    if(currentPlansView === 'monthly') {
        ...
        subtitle.innerHTML =
            'پلن های زیر بدون محدودیت زمانی ( بر اساس مقدار ترافیک ) و <b>مخصوص شرایط نت ملی ایران</b> هستند<br><br>یک پلن را انتخاب کنید — پرداخت با ⭐ استارز تلگرام ، <i class="fas fa-coins"></i> ارز دیجیتال ، <i class="fas fa-credit-card"></i> کارت به کارت';

        methods.innerHTML = `
        <div class="pm-badge stars"><i class="fas fa-star"></i> استارز تلگرام</div>
        ...
        warning.innerHTML = 
            `<span style="display:block; direction:rtl; text-align:right;">استارز تلگرام: فعال‌سازی حداکثر تا ۵ دقیقه از طریق ربات<br>ارز دیجیتال: تحویل فوری کلید (vless)<br>کارت به کارت: تحویل فوری کلید (vless)</span>`;
    ...
    grid.innerHTML = filteredTariffs.map((tariff) => {
        ...
        if (isMonthly) {
            ...
            if (tariff.gig > 1) {
                ...
                oldPrice = `
                <span class="tariff-old-price">
                        ${getToman(basePrice1v * tariff.gig).toFixed(0)} تومان
                        </span>
                `;
            }
        }

...
window.addEventListener('load', () => {
    ...
    currentLang = urlLang || localStorage.getItem('lang') || tgLang || 'en';
...
async function handleStarsPayment() {
    ...
    if (telegramId === 'DEV_USER') {
        showToast(
            ...
            ? 'لطفا از طریق تلگرام وارد شوید'
            : TRANSLATIONS[currentLang].go_telegram,
            'error'
        );
        return;
    }
...
window.startTrial = async () => {
    ...
    if (telegramId === 'DEV_USER') return showToast(TRANSLATIONS[currentLang].go_telegram, 'error');
